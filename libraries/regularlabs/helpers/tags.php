...
class RLTags
{
	...
	public static function getValuesFromString($string = '', $main_key = 'title', $known_boolean_keys = array(), $keep_escaped_commas = true)
	{
		...
		{
			self::unprotectSpecialChars($string, $keep_escaped_commas);
		...
		foreach ($matches as $match)
		{
			...
			self::unprotectSpecialChars($value, $keep_escaped_commas);
	...
	public static function unprotectSpecialChars(&$string, $keep_escaped_commas = false)
	{
		$unescaped_chars = array_keys(self::$protected_characters);

		if ($keep_escaped_commas)
		{
			array_walk($unescaped_chars, function (&$char)
			{
				$char = $char == ',' ? '\\' . $char : $char;
