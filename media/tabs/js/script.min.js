...
$el.tab('show');$el.closest('ul.nav-tabs').find('.rl_tabs-toggle').attr('aria-selected',false);$el.attr('aria-selected',true);$el.closest('div.rl_tabs').find('.tab-content').first().children().attr('aria-hidden',true);$('div#'+id).attr('aria-hidden',false);this.updateActiveClassesOnTabLinks($el);if(!slideshow){$el.focus();}},setScrollOnLoad:function($el){var self=this;if($el.hasClass('in')){self.scrollOnLoad();return;}
...
var scroll=false;var $anchor=$('#'+id+',a[name="'+id+'"],a#anchor-'+id+'');if(!$anchor.length){return;}
$anchor=$anchor.first();if(!$anchor.closest('.rl_tabs').length){return;}
var $tab=$anchor.closest('.tab-pane').first();var tab_id=$tab.attr('id');if($link.closest('.rl_tabs').length){if($link.closest('.tab-pane').first().attr('id')==tab_id){return;}}
